...
TEST(TestAssembleStiffnessMatrix, TwoTriangles) {
	...
	computeStiffnessMatrix(stiffnessMatrixLowerTriangle, vertices.row(0), vertices.row(1), vertices.row(2));

	Eigen::Matrix3d stiffnessMatrixUpperTriangle;
	computeStiffnessMatrix(stiffnessMatrixUpperTriangle, vertices.row(1), vertices.row(3), vertices.row(2));
	...
	ASSERT_EQ(stiffnessMatrixLowerTriangle(1, 1)
	              + stiffnessMatrixUpperTriangle(0, 0),
	          stiffnessMatrixFromAssembly.coeff(1, 1))
	    << "Wrong value at (1,1)";

	ASSERT_EQ(stiffnessMatrixLowerTriangle(1, 2)
	              + stiffnessMatrixUpperTriangle(0, 2),
	          stiffnessMatrixFromAssembly.coeff(1, 2))
	    << "Wrong value at (1,2)";
	;

	ASSERT_EQ(stiffnessMatrixLowerTriangle(2, 1)
	              + stiffnessMatrixUpperTriangle(2, 0),
	          stiffnessMatrixFromAssembly.coeff(2, 1));

	ASSERT_EQ(stiffnessMatrixLowerTriangle(2, 2)
	              + stiffnessMatrixUpperTriangle(2, 2),
	          stiffnessMatrixFromAssembly.coeff(2, 2));
}
